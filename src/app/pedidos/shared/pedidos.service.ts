...
gerarPedido(pedido: any) {
  ...
  private criarObjetoPedido(pedido: any) {
    ...
    let pedidoRef = {
      numero: numeroPedido,
      status: PedidosService.STATUS.ENVIADO,
      data: dataPedido,
      ...
      usuarioStatus: this.afAuth.auth.currentUser.uid + '_' + PedidosService.STATUS.ENVIADO,
      total: pedido.total
    }
    return pedidoRef;
  }
  ...
  getStatusNome(status: number) {
    switch (status){
      case PedidosService.STATUS.ENVIADO:
        return 'Aguardando confirmação';
      case PedidosService.STATUS.CONFIRMADO:
        return 'Em preparação';
      case PedidosService.STATUS.SAIU_PARA_ENTREGA:
        return 'Saiu para entregar';
      case PedidosService.STATUS.ENTREGUE:
        return 'Entregue';
    }
  }

  getFormaPagamento(paymentType: number) {
    switch(paymentType){
      case PedidosService.TIPO_FORMA_PAGAMENTO.DINHEIRO:
        return 'Dinheiro';
      case PedidosService.TIPO_FORMA_PAGAMENTO.CARTAO:
  ...
  getAllAbertos() {
    const usuarioStatus = this.afAuth.auth.currentUser.uid + '_' + PedidosService.STATUS.SAIU_PARA_ENTREGA;
    return this.db.list(FirebasePath.PEDIDOS,
